...
void GazeboControllerInterface::Load(physics::ModelPtr _model,
	...
	getSdfParam<std::string>(_sdf, "commandMotorSpeedSubTopic",
			command_motor_speed_sub_topic_, command_motor_speed_sub_topic_);
	...
	motor_velocity_reference_pub_ = node_handle_->advertise
			< mav_msgs::Actuators > (motor_velocity_reference_pub_topic_, 1);
}
...
void GazeboControllerInterface::OnUpdate(const common::UpdateInfo& /*_info*/) {
	...
	for (int i = 0; i < input_reference_.size(); i++) {
		turning_velocities_msg->angular_velocities.push_back(
				input_reference_[i]);
	}
...
void GazeboControllerInterface::CommandMotorCallback(
		const mav_msgs::ActuatorsConstPtr& input_reference_msg) {
