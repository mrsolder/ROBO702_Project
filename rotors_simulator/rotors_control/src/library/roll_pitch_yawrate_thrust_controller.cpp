...
void RollPitchYawrateThrustController::ComputeDesiredAngularAcc(Eigen::Vector3d* angular_acceleration) const {
  ...
  double xroll = roll_pitch_yawrate_thrust_.roll;
  double xpitch = roll_pitch_yawrate_thrust_.pitch;

  if(roll_pitch_yawrate_thrust_.roll>(25.0*3.141592654/180.0)){
	  xroll = 25.0;
  } else if(roll_pitch_yawrate_thrust_.roll<(-25.0*3.141592654/180.0)){
	  xroll = -25.0;
  }

  if(roll_pitch_yawrate_thrust_.pitch>(25.0*3.141592654/180.0)){
	  xpitch = 25.0;
  } else if(roll_pitch_yawrate_thrust_.pitch<(-25.0*3.141592654/180.0)){
	  xpitch = -25.0;
  }

  // Ismail Modification - End

  // Get the desired rotation matrix.
  Eigen::Matrix3d R_des;
  R_des = Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ())  // yaw
		* Eigen::AngleAxisd(xroll, Eigen::Vector3d::UnitX())  // roll			// Ismail Modification	Add
		* Eigen::AngleAxisd(xpitch, Eigen::Vector3d::UnitY());  // pitch			// Ismail Modification	Add
//        * Eigen::AngleAxisd(roll_pitch_yawrate_thrust_.roll, Eigen::Vector3d::UnitX())  // roll			// Ismail Modification	Rem
//        * Eigen::AngleAxisd(roll_pitch_yawrate_thrust_.pitch, Eigen::Vector3d::UnitY());  // pitch			// Ismail Modification	Rem
